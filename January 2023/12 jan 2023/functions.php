...
//Promotion strip Carousel
	...
	// });
			...
			$(document).on("click",".add_cart",function(e) {
				e.preventDefault();
				const url = 'https://jroll.com/?add-to-cart=697&quantity=1';
				$.ajax({
					type: 'GET',
					url:url,
...
add_filter( 'xoo_wsc_cart_totals' ,'cf_xoo_wsc_cart_totals');
function cf_xoo_wsc_cart_totals( $totals ){

	$totals['tax']['label'] = 'VAT';
	return $totals;

}

add_action( 'woocommerce_after_add_to_cart_button', 'cf_woocommerce_after_add_to_cart_button', 9 );
// function cf_woocommerce_after_add_to_cart_button( $data, $product, $variation ) {
function cf_woocommerce_after_add_to_cart_button($prodidshortcode) {
	// 
	// $data['availability_html'] .= 'TT5';
	// die('asfsf');
	echo 'TT5';
	if( class_exists('B2bking_Public') ) {
	echo ' <div class="tbl_start">';
		// $data['availability_html'] .= ' | TT6';
		// remove_filter( 'woocommerce_available_variation', array('B2bking_Public','b2bking_show_tiered_pricing_table_variation'), 10 );
if (!empty($prodidshortcode)) {
	echo '</div>testing';
}
		remove_action( 'woocommerce_after_add_to_cart_button', 'B2bking_Public::b2bking_show_tiered_pricing_table' );
	}

}

add_action( 'woocommerce_after_variations_form', 'cf_woocommerce_after_variations_form', 9 );
function cf_woocommerce_after_variations_form(){
	echo 'hellpo';
}